...
void DXUTParseCommandLine()
{
    ...
    for(;;)
    {
        ...
        if( _wcsnicmp( strCmdLine, strFlag, nFlagLen ) == 0 )
        {
            DXUTGetGlobalDialogResourceManager()->SetAutomation( true );
            strCmdLine += nFlagLen;
            continue;
        }
                           ...
                           D3DDISPLAYMODE* pAdapterDesktopDisplayMode )
{
	...
	int idm;
    for( idm = 0; idm < pDeviceSettingsCombo->pAdapterInfo->displayModeList.GetSize(); idm++ )
    {
                                   ...
                                   DXUTMatchOptions* pMatchOptions )
{
	...
	int i;
    for( i=0; i<pBestDeviceSettingsCombo->depthStencilFormatList.GetSize(); i++ )
    {
